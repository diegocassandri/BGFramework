...
app.directive("uiInputTime", ['$filter', function($filter){
	return {
		...
		link: function($scope, element, attrs){
			...
			$scope.dataFormat = app.lang.l["formats"]["time"];
			if ($scope.dataFormat){
				$scope.editDataFormat = $scope.dataFormat.split('H').join('=')
					.split('h').join('H')
					.split('=').join('h');	
			}
			
			$scope.dataMask = app.lang.l["masks"]["time"];
			...
			element.datetimepicker({
				...
				pickDate: false,
				format: $scope.editDataFormat,
				pick12HourFormat: false
			});	

			element.data('datetimepicker').setValue(null);
